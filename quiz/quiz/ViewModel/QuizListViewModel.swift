...
final class QuizListViewModel: QuizListProtocol {
    ...
    func getQuizList(completion: @escaping (String?) -> Void) {
        ...
        webService.getQuizList(pageSize: currentSizeCount, page: currentQuizPageCount,
                               categoryID: categoryId) { result, _  in
            switch result {
            case .success(let list):
                ...
                self.checkPaginateEnableQuiz(self.quizList.count, allItemsCount: list.count ?? 0)
                    ...
                    for index in self.quizList {
                        guard let image = index.image else { return }
    ...
    func search(searchText: String, completion: @escaping (Bool) -> Void) {
        ...
        webService.searchQuizs(searchText: searchText, categoryID: categoryID ?? 0) { result, _ in
            switch result {
